#' @title Retrieve only the best BLAST hit for each query
#' @description This function performs a BLAST search between query and subject sequences and
#' returns only the best hit based on the following criteria.
#' 
...
blast_best_hit <-
  ...
  if (search_type == "nucleotide_to_nucleotide") {
    ...
    blast_res <- blast_nucleotide_to_nucleotide(
      query = query,
      subject = subject,
      strand = strand,
    ...
    query_id <- NULL
    hit_tbl <- dplyr::do(dplyr::group_by(blast_res, query_id), get_best_hit(.))
  }
  
  if (search_type == "nucleotide_to_protein") {
    
    blast_res <- blast_nucleotide_to_protein(
      query = query,
      subject = subject,
      strand = strand,
      output.path = output.path,
  ...
  if (search_type == "protein_to_nucleotide") {
    ...
    blast_res <- blast_protein_to_nucleotide(
      query = query,
      subject = subject,
      output.path = output.path,
      is.subject.db = is.subject.db,
  ...
  if (search_type == "protein_to_protein") {
    ...
    blast_res <- blast_protein_to_protein(
      query = query,
      subject = subject,
      output.path = output.path,
      is.subject.db = is.subject.db,
