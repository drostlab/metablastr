#' @title Perform Protein to Nucleotide BLAST Searches (TBLASTN)
...
#' \code{\link{blast_nucleotide_to_protein}}, \code{\link{blast_best}}
#' @export

blast_protein_to_nucleotide <- function(query, 
                                        subject,
                                        output.path = NULL,
                                        is.subject.db = FALSE,
                                        task = "tblastn",
  ...
  if (!is.element(task, c("tblastn", "tblastn-fast")))
    stop("Please choose a protein-nucleotide comparison task that is supported by BLAST: task = 'tblastn' or task = 'tblastn-fast'.", call. = FALSE)
  
  message("Starting 'tblastn -task ", task,"' with  query: ", query, " and subject: ",subject," using ", cores, " core(s) ...")
  
  blast_call <-
    paste0("tblastn -query ", ws_wrap(query), " -db ", ws_wrap(subject))
