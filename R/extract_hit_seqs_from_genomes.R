...
extract_hit_seqs_from_genomes <-
  function(blast_tbl,
    ...
    if (!is.null(file_name) & separated_by_genome)
      stop("A file name can only be specified when separated_by_genome = 'FALSE'. Otherwise separate species specific fasta files are being generated.", call. = FALSE)
    ...
    if (separated_by_genome) {
      for (i in seq_len(length(subject_genomes))) {
        ...
        if (nrow(species_specific_blast_tbl) > 0) {
          ...
          chr_names <- dplyr::intersect(imported_genome_i_names, names(table(species_specific_blast_tbl$subject_id)))
          ...
          seq_file_paths[i] <- ifelse(
            is.null(path),
            file.path(getwd(), paste0(subject_genomes[i], "_blast_tbl_sequences.fa")),
            file.path(path, paste0(subject_genomes[i], "_blast_tbl_sequences.fa"))
          )
          
          # for each chromosome separately
          for (j in seq_len(length(chr_names))) {
            message("  -> Extracting BLAST hit sequences from ", chr_names[j], " ...")
            # divide by plus and minus strand
          
            species_specific_blast_tbl_plus_strand <-
              dplyr::filter(species_specific_blast_tbl, subject_id == chr_names[j], s_strand == "plus")
            species_specific_blast_tbl_minus_strand <-
              dplyr::filter(species_specific_blast_tbl, subject_id == chr_names[j], s_strand == "minus")
            
            if (!identical(unique(species_specific_blast_tbl_plus_strand$s_len),
      ...
      if(is.null(file_name)) {
        warning("No file name was specified, thus the default file name 'blast_hit_seqs.fa' was used.", call. = FALSE)
